...
APP_NAME = 'giSeriesDate'
APP_VERSION = '0.0.1'
APP_DESCRIPTION = 'Convert dates in iSeries format'
APP_ID = 'giseries_date.muflone.com'
APP_URL = 'http://www.muflone.com/giseries_date/'
APP_AUTHOR = 'Fabio Castelli'
APP_AUTHOR_EMAIL = 'muflone@vbsimple.net'
APP_COPYRIGHT = 'Copyright 2016 %s' % APP_AUTHOR
# Other constants
DOMAIN_NAME = 'giseries_date'
VERBOSE_LEVEL_QUIET = 0
VERBOSE_LEVEL_NORMAL = 1
VERBOSE_LEVEL_MAX = 2

# Paths constants
# If there's a file data/giseries_date.png then the shared data are searched
# in relative paths, else the standard paths are used
if os.path.isfile(os.path.join('data', 'giseries_date.png')):
    DIR_PREFIX = '.'
    DIR_LOCALE = os.path.join(DIR_PREFIX, 'locale')
    DIR_DOCS = os.path.join(DIR_PREFIX, 'doc')
else:
    DIR_PREFIX = os.path.join(sys.prefix, 'share', 'giseries_date')
    DIR_LOCALE = os.path.join(sys.prefix, 'share', 'locale')
    DIR_DOCS = os.path.join(sys.prefix, 'share', 'doc', 'giseries_date')
...
FILE_ICON = os.path.join(DIR_DATA, 'giseries_date.png')
